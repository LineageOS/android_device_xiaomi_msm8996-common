...
bool checkParam(struct pcm_params* params, pcm_param param, unsigned int value,
...
int sampleIsPlayable(unsigned int card, unsigned int device, unsigned int channels,
    ...
    auto canPlay = checkParam(params, PCM_PARAM_RATE, rate, "Sample rate", "Hz");
    canPlay &= checkParam(params, PCM_PARAM_CHANNELS, channels, "Sample", " channels");
    canPlay &= checkParam(params, PCM_PARAM_SAMPLE_BITS, bits, "Bitrate", " bits");
    canPlay &= checkParam(params, PCM_PARAM_PERIOD_SIZE, period_size, "Period size", " frames");
    canPlay &= checkParam(params, PCM_PARAM_PERIODS, period_count, "Period count", " periods");
...
void playSample(FILE* file, unsigned int card, unsigned int device, unsigned int channels,
                unsigned int rate, unsigned int bits, unsigned int period_size,
    ...
    if (!sampleIsPlayable(card, device, channels, rate, bits, period_size, period_count)) {
        return;
    }
...
bool playSound(const char* filename) {
    ...
    playSample(file, 0 /* card */, 0 /* device */, chunk_fmt.num_channels, chunk_fmt.sample_rate,
...
int main() {
    if (const auto mixer = mixer_open(0)) {
        // Enable speaker
        setMixerValueByName(mixer, "QUAT_MI2S_RX Audio Mixer MultiMedia1", 1);
        setMixerValueByName(mixer, "left Profile", 0 /*music*/);

        // Play amplifier calibration sound
        playSound("/vendor/etc/silence_short.wav");

        // Disable speaker
        setMixerValueByName(mixer, "QUAT_MI2S_RX Audio Mixer MultiMedia1", 0);
