...
static std::string get_scaled_duty_pcts(int brightness) {
    std::string buf;
    std::string pad;

    for (auto i : { 0, 12, 25, 37, 50, 72, 85, 100 }) {
        buf += pad;
...
void Light::setSpeakerLightLocked(const LightState& state) {
    ...
    if (blink) {
        ...
        mRedDutyPcts << get_scaled_duty_pcts(red) << std::endl;
        ...
        mGreenDutyPcts << get_scaled_duty_pcts(green) << std::endl;
        ...
        mBlueDutyPcts << get_scaled_duty_pcts(blue) << std::endl;
