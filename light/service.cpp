...
int main() {
    ...
    if (!redDutyPcts) {
        LOG(ERROR) << "Failed to open " << kRedDutyPctsPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream greenDutyPcts(kGreenDutyPctsPath);
    if (!greenDutyPcts) {
        LOG(ERROR) << "Failed to open " << kGreenDutyPctsPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream blueDutyPcts(kBlueDutyPctsPath);
    if (!blueDutyPcts) {
        LOG(ERROR) << "Failed to open " << kBlueDutyPctsPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream redStartIdx(kRedStartIdxPath);
    if (!redStartIdx) {
        LOG(ERROR) << "Failed to open " << kRedStartIdxPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream greenStartIdx(kGreenStartIdxPath);
    if (!greenStartIdx) {
        LOG(ERROR) << "Failed to open " << kGreenStartIdxPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream blueStartIdx(kBlueStartIdxPath);
    if (!blueStartIdx) {
        LOG(ERROR) << "Failed to open " << kBlueStartIdxPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream redPauseLo(kRedPauseLoPath);
    if (!redPauseLo) {
        LOG(ERROR) << "Failed to open " << kRedPauseLoPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream greenPauseLo(kGreenPauseLoPath);
    if (!greenPauseLo) {
        LOG(ERROR) << "Failed to open " << kGreenPauseLoPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream bluePauseLo(kBluePauseLoPath);
    if (!bluePauseLo) {
        LOG(ERROR) << "Failed to open " << kBluePauseLoPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream redPauseHi(kRedPauseHiPath);
    if (!redPauseHi) {
        LOG(ERROR) << "Failed to open " << kRedPauseHiPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream greenPauseHi(kGreenPauseHiPath);
    if (!greenPauseHi) {
        LOG(ERROR) << "Failed to open " << kGreenPauseHiPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream bluePauseHi(kBluePauseHiPath);
    if (!bluePauseHi) {
        LOG(ERROR) << "Failed to open " << kBluePauseHiPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream redRampStepMs(kRedRampStepMsPath);
    if (!redRampStepMs) {
        LOG(ERROR) << "Failed to open " << kRedRampStepMsPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream greenRampStepMs(kGreenRampStepMsPath);
    if (!greenRampStepMs) {
        LOG(ERROR) << "Failed to open " << kGreenRampStepMsPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }

    std::ofstream blueRampStepMs(kBlueRampStepMsPath);
    if (!blueRampStepMs) {
        LOG(ERROR) << "Failed to open " << kBlueRampStepMsPath << "(" << errno << "): " << strerror(errno);
        return -errno;
    }
