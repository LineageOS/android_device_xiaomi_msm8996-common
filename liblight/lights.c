...
static const char BUTTON_3_FILE[]
        ...
        struct light_state_t const* state)
{
    ...
    if (max_brightness != DEFAULT_MAX_BRIGHTNESS) {
        ...
        brightness *= max_brightness / DEFAULT_MAX_BRIGHTNESS;
        ...
        const struct light_state_t *state)
{
    ...
    pthread_mutex_lock(&g_lock);
    err = write_int(BUTTON_1_FILE, brightness) +
            write_int(BUTTON_2_FILE, brightness) +
            write_int(BUTTON_3_FILE, brightness);
    pthread_mutex_unlock(&g_lock);
    return err;
}
        ...
        struct hw_device_t** device)
{
    ...
    if (max_brightness < 0) {
        ALOGE("%s: failed to read max panel brightness, fallback to 255!", __func__);
