...
static int write_bdwlan_file(int need_fem_fix) {
    ...
    if (ISMATCH(hwversion_major, GEMINI)) {
        fprintf(fp, "%s", GEMINI_BDWLAN);
    ...
    } else if (ISMATCH(hwversion_major, SCORPIO)) {
        fprintf(fp, "%s\n", SCORPIO_BDWLAN);
