...
static int write_ff_flag_file(unsigned int* need_fem_fix) {
    FILE* fp;
...
static int write_bdwlan_file(unsigned int* need_fem_fix) {
    ...
    if (ISMATCH(hwversion_major, SCORPIO)) {
        ...
        if (need_fem_fix) {
            ...
            fprintf(fp, "%s", NATRIUM_BDWLAN);
               ...
               !ISMATCH(hwversion_major, GEMINI_LITE)) {
...
int main() {
    void* handle = NULL;
    unsigned int* need_fem_fix = 0; // Disabled by default
    ...
    qmi_nv_read_oem_item_3(&need_fem_fix);
